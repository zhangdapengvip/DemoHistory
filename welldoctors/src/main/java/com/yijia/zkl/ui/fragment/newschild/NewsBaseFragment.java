...
import com.yijia.zkl.ui.activity.NewsDetailActivity;
import com.yijia.zkl.ui.activity.WebviewActivity;
import com.yijia.zkl.ui.bean.Constants;
import com.yijia.zkl.ui.bean.OfficeInfo;
import com.yijia.zkl.ui.holder.NewsHolder;
...
import com.yijia.zkl.ui.view.ScrollingViewPager;
...
public abstract class NewsBaseFragment extends ListBaseFragment {
    ...
    protected void initData() {
        ...
        mDomain = loginInfo.getDatas().getDomain();
    ...
    private void loadList() {
        ...
        if (mPageNum == AppConstants.FIRST_NUM) {
            InformationPageResponse response = UtilsGson.fromJson(getSaveInfo(SAVE_KEY + mDomain),
        ...
        mSubscriber = RetrofitUtils.request(mActivity, ob,
                new RetrofitUtils.ResponseListener<InformationPageResponse>() {
                    ...
                    public void onSuccess(InformationPageResponse response) {
                        if (mPageNum == AppConstants.FIRST_NUM)
                            saveInfo(SAVE_KEY + mDomain, UtilsGson.toJson(response));
                        fillList(response);
                    }
    ...
    private void loadAdvert() {
        ...
        if (mPageNum == AppConstants.FIRST_NUM) {
            InformationHeadResponse response = UtilsGson.fromJson(getSaveInfo(SAVE_HEADKEY + mDomain),
        ...
        RetrofitUtils.request(mActivity, ob,
                new RetrofitUtils.ResponseListener<InformationHeadResponse>() {
                    ...
                    public void onSuccess(InformationHeadResponse response) {
                        if (mPageNum == AppConstants.FIRST_NUM)
                            saveInfo(SAVE_HEADKEY + mDomain, UtilsGson.toJson(response));
                        fillAdvert(response);
                    }
