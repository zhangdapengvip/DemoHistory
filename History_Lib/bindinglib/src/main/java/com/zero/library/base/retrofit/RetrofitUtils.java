...
public class RetrofitUtils {
    private static final String TAG_REQUEST = "requesting";

    public static final <T> Subscriber<? super T> request(Activity activity,
    ...
    public static final <T> Subscriber<? super T> request(Activity activity,
                                                          ...
                                                          ResponseListener<T> listener,
                                                          boolean showToast) {
        return request(activity, observable, null, listener, showToast);
    }

    public static final <T> Subscriber<? super T> request(final Activity activity,
                                                          Observable<T> observable,
                                                          final View view,
    ...
    public static final <T> Subscriber<? super T> request(final Activity activity,
        ...
        if (!UtilsUi.isNetworkConnected(activity)) {
            AppToast.show(activity, R.string.error_not_network);
        ...
        Subscriber<? super T> subs = new Subscriber<T>() {
            ...
            public void onError(Throwable e) {
                ...
                if (!TextUtils.isEmpty(e.getMessage()) && e.getMessage().contains("Unauthorized")) {
                    if (showToast) AppToast.show(activity, R.string.error_login_notice);
                } else {
                    if (showToast) AppToast.show(activity, R.string.error_service_response);
                }
            }
