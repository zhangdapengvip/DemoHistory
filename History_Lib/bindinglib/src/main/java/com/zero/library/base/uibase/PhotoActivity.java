 ...
 * 拍照父类
 */
public abstract class PhotoActivity extends AppBaseActivity implements AppBottomMenuWindow.OnMenuItemCheckListener {
    ...
    public void checkPosition(int position) {
        switch (position) {
            case PhotoMenuWindow.BUTTON_ONE:
                mImgUri = Uri.fromFile(new File(DirConstants.DEFAULT_IMG_DIR, +SystemClock.elapsedRealtime() + ".jpg"));
            ...
            case PhotoMenuWindow.BUTTON_TWO:
                JumpManager.doJumpForwardWithResult(mActivity, UtilsBitmap.getGalleryIntent(), CAMERA_LIBRARY);
                closePhotoWindow();
                break;
    ...
    protected abstract void imageBack(Uri uri, String path);
        ...
        if (resultCode == Activity.RESULT_OK) {
            switch (requestCode) {
                case CAMERA_LIBRARY:
                    ...
                    break;
                case TAKE_PHOTO:
                    imageBack(mImgUri, mImgUri.getPath());
                    break;
            }
        }
    }
