...
public class UtilsUi {
    ...
    public static PopupWindow showDropListPopuwindow(Activity activity,
                                                     View parentView, View content, boolean
                                                             canCancle) {
    ...
    public static void setRatingLayoutParams(RatingBar ratingBar) {
        LinearLayout.LayoutParams paramsRating = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.WRAP_CONTENT, (int) (UtilsUi.getDensity() * 15));
        ratingBar.setLayoutParams(paramsRating);
    }

    /**
     * 设置TextView内容适应view大小
     *
     * @param view      TextView
     * @param text      文本内容
     * @param textSize  字体大小
     * @param viewWidth TextView宽度
     */
    public static void resizeTextView(TextView view, String text, float textSize, int viewWidth) {
        view.setTextSize(TypedValue.COMPLEX_UNIT_PX, textSize);
        TextPaint paint = new TextPaint();
        paint.setTextSize(textSize);
        float strLength = paint.measureText(text);
        if (viewWidth < strLength) {
            resizeTextView(view, text, textSize - 0.5f, viewWidth);
            return;
        }
        view.setText(text);
    }
