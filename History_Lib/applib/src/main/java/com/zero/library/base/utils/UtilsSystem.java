...
public class UtilsSystem {
    ...
    public static List<String> getSystemLibs() {
        ...
        Logger.d("SystemLibs: " + listLibNames.toString());
        return listLibNames;
    }
    ...
    public static long getExternalTotalSpace() {
        long totalSpace = -1L;
        if (UtilsFile.isSDCardAvailable()) {
            try {
    ...
    public static long getUsedMemory(String packageName) {
        ...
        if (UtilsString.isEmpty(packageName)) {
    ...
    public static long getTotalMemory() {
        ...
        try {
            String totalMemory = UtilsFile.readProperties(path, "MemTotal",
                    null);// 读出来是带单位kb的，并且单位前有空格，所以去掉最后三位
            if (!UtilsString.isEmpty(totalMemory) && totalMemory.length() > 3) {
        ...
        for (RunningAppProcessInfo appProcess : appProcesses) {
            if (appProcess.processName.equals(context.getPackageName())) {
                if (appProcess.importance == RunningAppProcessInfo.IMPORTANCE_FOREGROUND) {
                    Logger.i(context.getPackageName(), "处于前台"
        ...
        for (RunningAppProcessInfo appProcess : appProcesses) {
            if (appProcess.processName.equals(context.getPackageName())) {
                if (appProcess.importance == RunningAppProcessInfo.IMPORTANCE_BACKGROUND) {
                    Logger.i(context.getPackageName(), "处于后台"
                            + appProcess.processName);
                    return true;
                }
            }
